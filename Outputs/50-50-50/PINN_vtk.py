...
# Train the model
history = model.fit(XYT, XYT, epochs=25, batch_size=1024, verbose=1, callbacks=[time_callback])
...
def save_vtk_files():
    ...
    for t in time_steps:
        ...
        # Get predictions
        prediction = model.predict(input_data, verbose=0).reshape(500, 500)
        
        # Write VTK file
        filename = f"pinn_t{t:.1f}.vtk"
        ...
        with open(filename, "w") as f:
            ...
            f.write(f"DIMENSIONS {500} {500} 1\n")  # 500x500 grid
            f.write(f"POINTS {500*500} float\n")
            ...
            for val in prediction.flatten(order='F'):  # Fortran-order for VTK
                f.write(f"{val}\n")
