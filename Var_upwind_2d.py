...
parser.add_argument('-scheme', choices=('uw','rk2' ), help='uw',
                    default='uw')
parser.add_argument('-ncellx', type=int, help='Number of x cells', default=100)
parser.add_argument('-ncelly', type=int, help='Number of y cells', default=100)
...
def init_plot(ax1, ax2, u0):
    ...
    ax1.set_ylabel('v')
    '''
    cp = ax2.contourf(xgrid, ygrid, u0, levels=16)
...
def update_plot(t, u1):
    ...
    cp = ax2.contourf(xgrid, ygrid, u1, levels=16)
...
while t < Tf:
    ...
    for i in range(2, nx+3):
        ...
        for j in range(2, ny+3):
            ...
            if (-y) > 0:
                if x>0:
                    v[i,j] = v_old[i,j] - lamx*(-y) *(v_old[i,j] - v_old[i-1,j]) - lamy*x *(v_old[i,j] - v_old[i,j-1])
                else:
                    v[i,j] = v_old[i,j] - lamx*(-y) *(v_old[i,j] - v_old[i-1,j]) - lamy*x *(v_old[i,j+1] - v_old[i,j])
            elif x>0:
                v[i,j] = v_old[i,j] - lamx*(-y) *(v_old[i+1,j] - v_old[i,j]) - lamy*x *(v_old[i,j] - v_old[i,j-1])
            else:
                v[i,j] = v_old[i,j] - lamx*(-y) *(v_old[i+1,j] - v_old[i,j]) - lamy*x *(v_old[i,j+1] - v_old[i,j])
